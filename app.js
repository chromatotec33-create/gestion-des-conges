...
const el = {
  ...
  envLabel: document.querySelector('#env-label'),
...
const workdays = (start, end) => { let n = 0; const cur = new Date(`${start}T00:00:00`); const last = new Date(`${end}T00:00:00`); while (cur <= last) { const day = cur.getDay(); if (day !== 0 && day !== 6) n += 1; cur.setDate(cur.getDate() + 1); } return n; };
...
function initClient() {
  ...
  state.supabase = createClient(env.NEXT_PUBLIC_SUPABASE_URL, env.NEXT_PUBLIC_SUPABASE_ANON_KEY);
...
function renderDemande() {
  ...
  const recalc = () => { const s = form.elements.start.value; const e = form.elements.end.value; form.elements.days.value = s && e && new Date(e) >= new Date(s) ? String(workdays(s, e)) : '0'; };
  ...
  form.addEventListener('submit', async (evt) => {
    ...
    const { error } = await state.supabase.from('conges').insert({ user_id: state.session.user.id, type, date_debut: start, date_fin: end, nb_jours: nb, statut: 'en_attente', commentaire_public: form.elements.commentaire_public.value || null, commentaire_prive: form.elements.commentaire_prive.value || null });
...
function renderValidation() {
  ...
  if (!['chef_service', 'direction', 'admin'].includes(role || '')) { el.validation.innerHTML = '<p>Vous n\'avez pas les droits de validation.</p>'; return; }
  ...
  const update = async (id, decision) => {
    ...
    if (!error) { await state.supabase.from('historiques').insert({ conge_id: id, action: decision === 'ok' ? 'validation' : 'refus', auteur_id: state.session.user.id, commentaire: comment }); await refreshData(); renderDashboard(); renderValidation(); }
  };
...
async function loadSession() {
  ...
  el.envLabel.textContent = client ? `Env: ${env.APP_BASE_URL || 'set'}` : 'Env Supabase manquant sur Vercel';
  if (!client) return;
...
el.loginForm.addEventListener('submit', async (e) => {
  ...
  initClient();
  if (!state.supabase) return void (status.textContent = 'Variables Vercel manquantes');
