...
async function fetchRequests(companyId: string, employeeId?: string): Promise<LeaveRequestRow[]> {
  ...
  let query = supabase
    .from("leave_requests")
    .select("id, leave_type_id, status, created_at, employee_id, leave_request_days(leave_date)")
      ...
      {!companyId ? (
        <Card>
          <p className="text-sm text-muted-foreground">
            Renseignez <code>companyId</code> dans l’URL pour charger les demandes réelles.
          </p>
        </Card>
      ) : null}
